...
int conditional(int x, int y, int z) {
  int mask = !!x;        // 将 x 转化为布尔值
  mask = ~mask + 1;      // 布尔值扩展为全0或全1的掩码
  return (mask & y) | (~mask & z);  // 根据掩码选择 y 或 z
}
/* 
...
int isLessOrEqual(int x, int y) {
  ...
  int signDiffResult = diff >> 31 & 1; // diff 的符号位
  
  return (signDiff & signX) | (!signDiff & !signDiffResult);
}
//4
/* 
